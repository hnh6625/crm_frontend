    ...
    <div class="toolbar card">
      <div class="toolbar-filters">
        <el-input
          ...
          @input="debounceFetch"
        />
        <el-select v-model="filters.source" placeholder="Nguồn" clearable style="width: 140px">
          <el-option
        ...
        <el-select v-model="filters.status" placeholder="Trạng thái" clearable style="width: 140px">
          <el-option
      ...
      <div class="toolbar-actions">
        ...
        <el-button v-if="authStore.isAdmin" @click="openAssignDialog">
          <span class="icon icon-sm">assignment_ind</span>
          Phân công
        </el-button>
        ...
        <el-table-column label="" width="80" fixed="right" @click.stop>
          <template #default="{ row }">
            <el-dropdown trigger="click" @click.stop>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item @click.stop="router.push('/leads/' + (row.id || row.leadId))">
...
onMounted(async () => {
  ...
  console.log("Sources =", JSON.stringify(leadStore.sources))
  console.log("Statuses =", JSON.stringify(leadStore.statuses))
  console.log("Tags =", JSON.stringify(leadStore.tags))
...
function openForm(lead) {
  console.log("LEAD CLICK EDIT =", lead)
...
async function doAssign() {
  ...
  try {
    await Promise.all(
      selectedRows.value.map(r => leadStore.assign(getLeadId(r), {consultantId: assignedTo.value}))
    )
